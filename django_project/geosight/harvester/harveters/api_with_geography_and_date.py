...
class APIWithGeographyAndDate(BaseHarvester):
    ...
    def _process(self):
        ...
        try:
            ...
            keys_for_geography_identifier = self.attributes[
                'keys_for_geography_identifier']
            ...
            try:
                ...
                if date_format:
                    date_data = datetime.strptime(
                        date_data, date_format).date()
                else:
                    try:
                        date_data = datetime.fromtimestamp(date_data).date()
                    except ValueError:
                        date_data = datetime.fromtimestamp(
                            int(date_data) / 1000).date()
                    except TypeError:
                        raise HarvestingError(
                            (
                                'The date is not in timestamp, '
                                'so needs to put time format in attributes.'
                            )
                        )
                ...
                if geography_identifier:
                    ...
                    self._update(
                        f'Geometry {geography_identifier} does not exist')
