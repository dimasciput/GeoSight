...
from geosight.data.models import (
    Indicator, AggregationMethod, IndicatorGroup
)
...
class DatasetListApiTest(BasePermissionTest, TestCase):
    ...
    def create_value(
            ...
            date_str,
            admin_level, geom_identifier, value
    ):
        """Create Indicator Value."""
        indicator.save_value(
            datetime.strptime(date_str, '%Y-%m-%d'), geom_identifier, value,
    ...
    def test_list_api_by_admin(self):
        ...
        # by codes
        response = self.assertRequestGetView(
            f'{url}?geom_identifier__in=A,B', 200, user=user
        )
